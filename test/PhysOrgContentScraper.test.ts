...
import {PhysOrgContentScraper} from "../src/scrapers/PhysOrgContentScraper";
import { expect } from 'chai';

require('dotenv').config();

describe('PhysOrgContentScraper 1', function () {
    describe('PhysOrgContentScraper 2', function () {
        this.timeout(9999999);

        const date = new Date()
        const testIndex = {} as ScrapingIndexI
        testIndex.scraperId = "test"
        const scraper = new PhysOrgContentScraper("test","");

        it('PhysOrgContentScraper', async function () {

            const url = "https://phys.org/news/2024-03-electronic-device-thermal-simpler-slightly.html"
            const result = await scraper.extractNewInUrl(url, "", 0, 0);
            console.log(result);
            expect(result).to.have.property("content")
            expect(result).to.have.property("contentMarkdown")
