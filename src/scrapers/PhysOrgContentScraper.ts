...
export class PhysOrgContentScraper extends ContentScraper {
    public newspaper: string
    public scraperId: string
    public excludedParagraphsEqual: string[] = [' ', '  ', ' \n', '  \n']
    public excludedParagraphsIncluding: string[] = ['Share this',]

    public mustStartWith = "https://phys.org/"
    ...
    async extractDate(): Promise<Date> {
        try {
            const element  = await this.page.$('script[type="application/ld+json"]');
            const text     = await this.page.evaluate(element => (element as any).innerText, element);
            const JSONparsedText = JSON.parse(text)
            const dateStr = JSONparsedText.datePublished
            console.log(dateStr)
    ...
    async extractDescription() {
        try {
            const description = await this.page.$eval("head > meta[property='twitter:description']", (element: any) => element.content);
            return description
        } catch (e) {
            return null
        }
    ...
    async extractAuthor() {
        try {
            let headline = await this.page.$eval("head > meta[name='twitter:data1']", (element: any) => element.content);
            return headline
        } catch (e) {
            return null
        }
